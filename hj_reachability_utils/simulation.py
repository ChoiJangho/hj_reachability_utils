...
def rollout_controller(x0, dynamics, controller, t_simulation, dt=0.005, exit_event_func=None):
    ...
    while t < t_simulation:
        ...
        for key, value in extras.items():
            if key in extras_combined:
                extras_combined[key].append(value)
            else:
                extras_combined[key] = [value]
    ...
    for key, value in extras.items():
        if key in extras_combined:
            ...
            extras_combined[key] = [value]
    return np.asarray(ts), np.asarray(xs).T, np.asarray(us).T, np.asarray(dxs).T, extras_combined